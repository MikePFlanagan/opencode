...
export function Session() {
  ...
  createEffect(() => {
    ...
    if (current === 0) return
    // Appends reset this timer, so backfill waits for a pause in streaming.
    const timer = setTimeout(() => {
      const before = scroll && !scroll.isDestroyed ? scroll.scrollHeight : undefined
      // Sticky scroll holds the bottom through the mount; compensation is only for readers
      // who have scrolled up.
      const atBottom = before === undefined || scroll.scrollTop + scroll.viewport.height >= before - 1
      setHiddenRows(Math.max(0, current - TRANSCRIPT_BACKFILL_CHUNK))
      if (atBottom || before === undefined) return
